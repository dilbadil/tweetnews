...
if ($handle == 1 && isset($_SESSION['flush'])) $handle = 2;
if ($handle == 3 && isset($_SESSION['flush']) && $start > count($_SESSION['flush'])) $handle = 2;
if ($handle == 1) {
  ...
  if ($output['success']) {
    ...
    if ($output['success']) {
      ...
      $_SESSION['flush'] = $data_follower_ids['ids'];
      $_SESSION[$search.'_follower_cursor'] = $data_follower_ids['next_cursor_str'];
      $output = tw_multi_user($search, $start, 'flush');
      if ($output['success']) {
        $data = $output['response'];
        foreach ($data as $user) {
          $json_table[] = $user;
        }
        $total = count($_SESSION['flush']);
  ...
  if ($output['success']) {
    $data_follower_ids = $output['response'];
    $_SESSION['flush'] = array_merge($_SESSION['flush'], $data_follower_ids['ids']);
    $_SESSION['flush'] = array_slice($_SESSION['flush'], 0, MAX_RET_DATA);
    $_SESSION[$search.'_follower_cursor'] = $data_follower_ids['next_cursor_str'];
    $output = tw_multi_user($search, $start, 'flush');
    if ($output['success']) {
      $data = $output['response'];
