    ...
    public function reconfigure($config=array()) {
      ...
      $this->config = array_merge(
        array(
          ...
          'user_agent'                 => 'JP Project App',
          'host'                       => 'api.twitter.com',

          'consumer_key'               => 'DbvCncKX3PbRIWQhXXiV8Q',
          'consumer_secret'            => 'f4F5seKzVPSsreySMY0iPFcSDQFZcS7wmQX24CtVY',
          'token'                      => '2232153978-SnNT7WNf8qnjEPTCevP73pWLd5yo1gHLn4x0JvV',
          'secret'                     => 'SG5vBAeaq7e1wIYAUMTAa8sVsseOhajj5bNRanWS1qhsn',
//~
          //~ 'consumer_key'               => 'CGR0vKnKT4GQuWPJgemSQ',
          //~ 'consumer_secret'            => 'zOXJhUi6y3Jifwk4N4y1k8sc1ejha9T3cLQVjwuGnx0',
          //~ 'token'                      => '',
          //~ 'secret'                     => '',
     ...
     */
    public function request($method, $url, $params=array(), $useauth=true, $multipart=false, $headers=array(), $multi = false) {
      $options = array(
        'method'    => $method,
        'url'       => $url,
        'params'    => $params,
    ...
    private function curlit() {
      ...
      if (isset($this->request_settings['multi']) && (true === $this->request_settings['multi'])) {
        $this->conn[] = $c;
        return 0;
      }
      // do it!
    ...
    public function exec_multi() {
      ...
      foreach ($this->conn as $channel) {
        $resp = curl_multi_getcontent($channel);
        curl_multi_remove_handle($mh, $channel);
        curl_close($channel);
        $resp = json_decode($resp);
        if (isset($resp->errors)) {
          $resps[] =  $resp->errors[0]->code;
        } else {
          $resps[] = 1;
        }
      }
