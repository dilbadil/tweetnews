...
while ($http_code != 200 || !empty($error))
{
  $connection->request('GET', $connection->url('1.1/followers/ids'),
    array(
    'cursor' => $_SESSION[$search.'follower_cursor'],
    'count' => 5000,
    'screen_name' => $search
  ));
...
if ($http_code == 200 && empty($error)) {
  ...
  $currentCount = count($data['ids']);
  $_SESSION[$search.'follower_ids'] = array_merge($_SESSION[$search.'follower_ids'],$data['ids']);
  $_SESSION[$search.'follower_cursor'] = $data['next_cursor_str'];
}
$count = 0;
$http_code = "";
while ($http_code != 200 || !empty($error))
...
$connection->request('GET', $connection->url('1.1/users/lookup'),
  ...
  'user_id' => array_slice($_SESSION[$search.'follower_ids'], $start, $limit)
));
$http_code = $connection->response['code'];
$error =$connection->response['error'];
if (++$count == 5)
