...
while ($http_code != 200 || !empty($error))
{
  $connection->request('GET', $connection->url('1.1/friends/ids'),
    array(
    'cursor' => $_SESSION[$search.'following_cursor'],
    'count' => 5000,
    'screen_name' => $search
  ));
...
if ($http_code == 200 && empty($error)) {
  ...
  $_SESSION[$search.'following_ids'] = array_merge($_SESSION[$search.'following_ids'],$data['ids']);
  $_SESSION[$search.'following_cursor'] = $data['next_cursor_str'];
}
$count = 0;
$http_code = "";
while ($http_code != 200 || !empty($error))
